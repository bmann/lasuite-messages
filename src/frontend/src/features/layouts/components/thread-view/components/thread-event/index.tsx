...
): event is TypedThreadEvent<"assign" | "unassign"> =>
 ...
 */
export const groupSystemEvents = (
...
export const isCondensed = (
  ...
  if (
    event.type !== ThreadEventTypeEnum.im ||
    previousEvent.type !== ThreadEventTypeEnum.im
  )
    return false;
...
export const ThreadEvent = ({
  ...
  if (isIMEvent(event)) {
    ...
    const imClasses = clsx("thread-event", "thread-event--im", {
      ...
      <div className={imClasses} id={`thread-event-${event.id}`}>
        ...
        >
          ...
          >
            ...
            {isEdited && (
              <span className="thread-event__edited-badge">
                ({t("edited")})
              </span>
            )}
          </div>
...
const SystemEventLine = ({ event, currentUserId }: SystemEventLineProps) => {
  ...
  const timeLabel = DateHelper.formatEventTimestamp(
    event.created_at,
    i18n.resolvedLanguage,
  );
    ...
    <div className="thread-event thread-event--collapsed">
      ...
      >
        <Icon
          ...
          aria-hidden="true"
        />
        <span className="thread-event__collapsed-summary">{summary}</span>
