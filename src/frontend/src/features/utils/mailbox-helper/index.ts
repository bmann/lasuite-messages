...
class MailboxHelper {
     ...
     * record left by another user, a corrupted one, or a storage that cannot
     * be accessed (blocked site data, sandboxed frame).
     */
    ...
    static persistLastActiveMailbox(userId: string, mailboxId: string): void {
        ...
        try {
            ...
            // A blocked or full storage must never break the mailbox switch.
        }
    }
    ...
    static clearLastActiveMailbox(): void {
        try {
            ...
            // Same tolerance as `clearPersistedWindows`: storage failures never block logout.
        }
    }
